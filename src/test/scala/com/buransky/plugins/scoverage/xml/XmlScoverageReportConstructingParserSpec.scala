...
import com.buransky.plugins.scoverage.{
  DirectoryStatementCoverage,
  FileStatementCoverage,
  NodeStatementCoverage,
...
class XmlScoverageReportConstructingParserSpec extends FlatSpec with Matchers {
  ...
  private def assertReportFile(data: String, expectedCoverage: Double, pathSanitizer: PathSanitizer)(
    f: (ProjectStatementCoverage) => Unit
  ) {
  ...
  private def checkNode(
    node: NodeStatementCoverage,
    name: String,
    count: Int,
    covered: Int,
    rate: Double
  ): Iterable[NodeStatementCoverage] = {
