...
class ScalastyleRunner(rp: RulesProfile) {
  ...
  def run(encoding: String, files: java.util.List[File]): List[Message[FileSpec]] = {
    ...
    val messages = new ScalastyleChecker[FileSpec]().checkFiles(config, fileSpecs.asScala)
  ...
  private def ruleToChecker(activeRule: ActiveRule): ConfigurationChecker = {
    val sonarParams = activeRule.getActiveRuleParams.asScala.map(p => (p.getKey, p.getValue)).toMap

    // TODO: Fix me checkerParams not used
