...
class ScalastyleRepositorySpec extends FlatSpec with Matchers with Inspectors with BeforeAndAfterAll {
  ...
  it should "give a name to every rule" in {
    rules.asScala.filter(_.name == null) should be(empty)
  }

  it should "set the ClazzParam for every rule" in {
    rules.asScala.filter(_.param(Constants.ClazzParam) == null) should be(empty)
  }
  ...
  it should "determine the parameter of a rule with a parameter" in {
    val rule = rules.asScala.find(_.key == "scalastyle_ParameterNumberChecker")
    rule.get.params.asScala map (_.key) shouldEqual List("maxParameters", Constants.ClazzParam)
  }
  ...
  it should "determine correct type of regex parameters" in {
    val rule = rules.asScala.find(_.key == "scalastyle_ClassTypeParameterChecker")
