...
class ScalastyleResourcesSpec extends FlatSpec with Matchers with Inspectors with PrivateMethodTester {
  ...
  it should "get scalastyle_definition.xml from Scalastyle jar" in {
    val xmlFromClassPath = PrivateMethod[Elem]('xmlFromClassPath)
    val scalastyleDefinitions = ScalastyleResources invokePrivate xmlFromClassPath(
  ...
  it should "get scalastyle_documentation.xml from Scalastyle jar" in {
    val xmlFromClassPath = PrivateMethod[Elem]('xmlFromClassPath)
    val scalastyleDocumentation = ScalastyleResources invokePrivate xmlFromClassPath(
      "/scalastyle_documentation.xml"
    )
  ...
  it should "return all defined checkers" in {
    ScalastyleResources.allDefinedRules.size shouldEqual 69
  }

  it should "give rules a description" in {
    forAll(ScalastyleResources.allDefinedRules) { r: RepositoryRule =>
      r.description.length should be > 0
    }
  }
  ...
  it should "get all parameters of rules with a parameter" in {
    val rule =
      ScalastyleResources.allDefinedRules.find(_.clazz == "org.scalastyle.scalariform.ParameterNumberChecker")
  ...
  it should "get all parameters of rules with multiple parameters" in {
    val rule =
      ScalastyleResources.allDefinedRules.find(_.clazz == "org.scalastyle.scalariform.MethodNamesChecker")
